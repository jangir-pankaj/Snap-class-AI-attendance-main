...
def student_dashboard():
    ...
    with c2:
        if st.button('Enroll Your Subject',type='primary',width='stretch'):
            enroll_dialog()
...
def student_screen():
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input('Enter your name ',placeholder='E.g Pankaj Jangir')
            st.subheader('Optional : Voice Enrollment')
            st.info('Enroll for voice only attendence')

            audio_data = None
            try:
                audio_data = st.audio_input('Record a short phrase like I am Present, My name is Pankaj')
            except Exception:

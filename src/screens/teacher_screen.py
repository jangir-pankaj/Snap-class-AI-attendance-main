...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button('Log Out',type='secondary',key='loginbackbutton',shortcut='control+backspace'):
...
def teacher_tab_take_attendence():
    ...
    if not subjects:
        st.warning("You have't created any subject yet ! Please create one to begin..")
        return
        ...
        if st.button('Run Face Analysis',type='secondary',width='stretch',icon=':material/analytics:',disabled= not has_photos):
            with st.spinner("Deep Scanning Classroom Photos..."):
                ...
                for idx,img in enumerate(st.session_state.attendence_images):
                    ...
                    result =  predict_attendence(img_np)
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username =  st.text_input("Enter the username",placeholder='@abhishek')
    teacher_password = st.text_input("Enter the password",type='password',placeholder="Enter password")

    st.divider()
    button1,button2 = st.columns(2)
    with button1:
        if st.button('Login',icon=":material/passkey:",icon_position='left',shortcut="control+enter",width='stretch'):
...
def teacher_screen_register():
    ...
    teacher_username =  st.text_input("Enter the username",placeholder='@abhishek')
    teacher_name = st.text_input("Enter name",placeholder="Abhishek")
    teacher_password = st.text_input("Enter the password",type='password',placeholder="Enter password")
    ...
    button1,button2 = st.columns(2)
    with button1:
        if st.button('Register Now',icon=":material/passkey:",icon_position='left',width='stretch',shortcut="control+enter"):
    ...
    with button2:
        if st.button('Login Instead',icon=":material/passkey:",type='primary',icon_position='left',width='stretch'):
            st.session_state.teacher_login_type = "login"
            st.rerun()

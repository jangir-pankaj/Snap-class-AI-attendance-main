...
def check_teacher_exists(username):
    # check for unique username , returns false if the username is already taken
...
def teacher_login(teacher_username,teacher_password):
    response = supabase.table("teachers").select("*").eq("username",teacher_username).execute()
    if response.data:
        teacher = response.data[0]
        if check_pass(teacher_password,teacher["password"]):
            return teacher
    return None
